<%@ Page Title="" Language="C#" MasterPageFile="~/hrms/HRMS.Master" AutoEventWireup="true" CodeBehind="promotion-entry.aspx.cs" Inherits="SigmaERP.hrms.payroll.promotion_entry" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                                                    ...
                                                    <div class="modal-dialog modal-xl">
                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-header">
                                                                <h5 class="modal-title" id="salaryModalLabel">Set Employee Promotion</h5>
        ...
        <script>
            ...
            var getDesignationUrl = `${rootUrl}/api/Designation/basicInfo?CompanyId=${CompanyID}`;
    ...
    `;
        ...
        `;
                ...
                ApiCall(getDepartmentUrl, token)
                    .then(function (response) {
                        if (response.statusCode === 200) {
                            ...
                            DepartmentPopulateDropdown(responseData);
                            console.log('after Table Data Bind ', responseData);
                        } else {
                            console.error('Error occurred while fetching data:', response.message);
                        }
            ...
            function GetDesignation() {
                ApiCall(getDesignationUrl, token)
                    .then(function (response) {
                        if (response.statusCode === 200) {
                            var responseData = response.data;
                            console.log('Before table Data Bind', responseData);
                            DesignationPopulateDropdown(responseData);
                            console.log('after Table Data Bind ', responseData);
                        } else {
                            console.error('Error occurred while fetching data:', response.message);
                        }
            ...
            function GetGroup(dptId, selectedGidId) {
                ApiCall(`${getGroupUrl}&DptId=${dptId}`, token)
                    .then(function (response) {
                        if (response.statusCode === 200) {
                            var responseData = response.data;
                            console.log('Before table Data Bind', responseData);
                            GroupPopulateDropdown(responseData);
                            const ddlGroup = document.getElementById('ddlGroup');
                            if (ddlGroup) {
                                const optionExists = Array.from(ddlGroup.options).some(opt => opt.value === selectedGidId);

                                // If exists, select it; otherwise select the default "0" option
                                ddlGroup.value = optionExists ? selectedGidId : '0';
            ...
            function GroupPopulateDropdown(data) {
                const dropdown = document.getElementById('ddlGroup');
            ...
            function DepartmentPopulateDropdown(data) {
            ...
            function DesignationPopulateDropdown(data) {
                const dropdown = document.getElementById('ddlDesignation');
                dropdown.innerHTML = '<option value="0">---Select---</option>'; // Clear existing options
                    ...
                    .then(function (response) {
                        if (response.statusCode === 200) {
                           ...
                           $('#ddlOvertTime').val(data.isSingleRateOT ? '' : (data.isOverTime ? '1' : '0'));
                            ...
                            $('#ddlDesignation').val(data.dsgId ?? '0');
                            $('#empDepartment').text(data.dptName ?? 'N/A');
                            let selectedDptId = data.dptId ?? '0';
                            $('#ddlDepartment').val(selectedDptId);

                            // Call function if not default
                            if (selectedDptId !== '0') {
                            ...
                            $('#empImage').attr('src', imagePath);
                             ...
                             selectedGidId = data.gId ?? '0';
                            // $('#ddlGroup').val(selectedGidId);
                        }
                    })
                    .catch(function (error) {
                        console.error("Error loading employee info:", error);
