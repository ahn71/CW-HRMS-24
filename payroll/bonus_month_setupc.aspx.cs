...
namespace SigmaERP.payroll
{
    public partial class bonus_month_setupc : System.Web.UI.Page
    {
        ...
        private void loadSlabType()
        {
            ...
            sqlDB.fillDataTable("select * from v_Payroll_BonusSetup_DistinctRecord1 where CompanyId='" + CompanyId + "' order by Year desc", dt);
        ...
        private bool chekHasRecord()
        {
            ...
            {
                DataTable dt = new DataTable();
                sqlDB.fillDataTable("select * from Payroll_BonusMonthSetup1 where BId=" + dlSelectBonusYearAndType.SelectedItem.Value.ToString() + "", dt = new DataTable());
                if (dt.Rows.Count > 0)
                {
                    // divPagelist.Visible = true;
                    gvBonusMonthList.DataSource = dt;
        ...
        protected void btnSet_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SQLOperation.forDeleteRecordByIdentifier("Payroll_BonusMonthSetup1", "BId", dlSelectBonusYearAndType.SelectedValue.ToString(), sqlDB.connection);
                for (byte b = 0; b < gvBonusMonthList.Rows.Count; b++)
                {
                    ...
                    if (SQLOperation.forSaveValue("Payroll_BonusMonthSetup1", getColumns, getValues, sqlDB.connection) == true)
                    {
                        status = true;
                    }
                }
        ...
        {
            lblMessage.InnerText = "";
            SQLOperation.forDeleteRecordByIdentifier("Payroll_BonusMonthSetup1", "BonusType", dlSelectBonusYearAndType.SelectedItem.ToString(), sqlDB.connection);
            gvBonusMonthList.DataSource = null;
            gvBonusMonthList.DataBind();
        }
        ...
        private void loadSetupedInfo()
        {
            ...
            {
                DataTable dt = new DataTable();
                sqlDB.fillDataTable("select distinct BonusType,Format(setupedDate,'dd-MMM-yyyy') setupedDate,convert(int,Year) as Year from v_Payroll_BonusMonthSetup1 order by convert(int,Year)", dt);
