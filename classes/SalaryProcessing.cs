...
namespace SigmaERP.classes
{
    public class SalaryProcessing
    {
                ...
                if (dtEmployees != null && dtEmployees.Rows.Count > 0)
                {
                    ...
                    if (IsSeperationGeneration == "1")
                    {
                        if (generateFor == "regular")
                            ...
                            salarySheetClearForSeparation_complaince(ToDate, CompanyId, EmpIdS);
        ...
        private void salarySheetClearForSeparation_complaince(DateTime ToDate, string CompanyId, string EmpIds)
        {
            try
            {
                //EmpId = (EmpId == "0") ? "" : " and EmpId ='" + EmpId + "'";
                CRUD.Execute("delete from Payroll_monthlysalarysheet_Compliances where  WHERE CompanyId = '" + CompanyId + @"' 
               AND YearMonth = '" + ToDate.ToString("yyyy-MM") + @"-01' 
               AND EmpId IN (" + EmpIds + ")");
            }
            catch { }
