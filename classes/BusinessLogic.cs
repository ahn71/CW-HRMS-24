            ...
            try
            {
                ...
                if (index == 0)                
                    query = "SELECT  EmpId, substring(EmpCardNo,8,15)  + ' (' + EmpProximityNo + ')' AS EmpCardNo, PSftName as SftName,EmpName,Address,GId,GName,PSftId as SftId, SUM(CASE DATEPART(day, AttDate) WHEN 1 THEN code ELSE 0 END) AS [1], SUM(CASE DATEPART(day, AttDate) WHEN 2 THEN code ELSE 0 END) AS [2]," +
                         ...
                         "(SUM(case Code when 207  then 1 else 0 end)+SUM(case Code when 223  then 1 else 0 end)+SUM(case Code when 205  then 1 else 0 end)+SUM(case Code when 217  then 1 else 0 end)+SUM(case Code when 217  then 1 else 0 end)) as LV ,sum(NightAllowCount) as MonthlyTotalOT" +
