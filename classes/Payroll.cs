...
namespace SigmaERP.classes
{
    ...
    "GKvbeŸB", "weivbeŸB", "wZivbeŸB", "PyivbeŸB", "cuPvbeŸB", "wQh়vbeŸB", "mvZvbeŸB", "AvUvbeŸB", "wbivbeŸB"};
        ...
        public static void loadMonthIdByCompanyForComplaince(DropDownList ddlMonthList, string CompanyId)
        {
            try
            {
                DataTable dt = new DataTable();
                sqlDB.fillDataTable("select Distinct case when FromDate is null then  Format(YearMonth,'MMM-yyyy') else Format(YearMonth,'MMM-yyyy')+' [ '+ convert(varchar(10),FromDate,120)+' to '+ convert(varchar(10),ToDate,120)+' ]'  end  as YearMonth,case when FromDate is null then convert(varchar(10), YearMonth, 120) else   convert(varchar(10), YearMonth, 120) + '/' + convert(varchar(10), FromDate, 120) + '/' + convert(varchar(10), ToDate, 120) end as MonthYear, year(YearMonth), month(YearMonth) from Payroll_monthlysalarysheet_Compliances where CompanyId = '" + CompanyId + "' order by year(YearMonth) desc, month(YearMonth) desc", dt);

                string hhh = "select Distinct case when FromDate is null then  Format(YearMonth,'MMM-yyyy') else Format(YearMonth,'MMM-yyyy')+' [ '+ convert(varchar(10),FromDate,120)+' to '+ convert(varchar(10),ToDate,120)+' ]'  end  as YearMonth,case when FromDate is null then convert(varchar(10), YearMonth, 120) else   convert(varchar(10), YearMonth, 120) + '/' + convert(varchar(10), FromDate, 120) + '/' + convert(varchar(10), ToDate, 120) end as MonthYear, year(YearMonth), month(YearMonth) from Payroll_monthlysalarysheet_Compliances where CompanyId = '" + CompanyId + "' order by year(YearMonth) desc, month(YearMonth) desc";
                ddlMonthList.DataSource = dt;
                ddlMonthList.DataValueField = "MonthYear";
                ddlMonthList.DataTextField = "YearMonth";
                ddlMonthList.DataBind();
        ...
        public static void loadBonusList_Compliance(DropDownList ddlBonus, string CompanyId)
        {
            try
            {
                DataTable dt = new DataTable();
                sqlDB.fillDataTable("select BId,BonusName from Payroll_BonusSetup1 where Status=1 and CompanyId='" + CompanyId + "' order by CalculationDate desc", dt);
                ddlBonus.DataSource = dt;
                ddlBonus.DataTextField = "BonusName";
                ddlBonus.DataValueField = "BId";
                ddlBonus.DataBind();
